...
class Truck extends Vehicle implements AbleToTow {
  ...
  constructor(vin: string, color: string, make: string, model: string, year: number, weight: number, topSpeed: number, wheels: Wheel[], towingCapacity: number){
    ...
    if(this.wheels.length !== 4){ // TODO: The constructor should check if the wheels array has 4 elements and create 4 new default Wheel objects if it does not
  ...
  tow(vehicle: Truck | Motorbike | Car): void {
    ...
    if (vehicle.make && vehicle.model) {
      console.log(`The vehicle is a ${vehicle.make} ${vehicle.model}`);
    }
